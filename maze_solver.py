...
class MazeSolver:
    ...
    def _maze_traversal(self, maze, current_x, current_y):
        ...
        if maze[current_x][current_y] == ".":
            ...
            # Calls separate function to print the maze
        ...
        # Check down
        if self.finish_flag is not True and maze[current_x + 1][current_y] == ".":
            self._maze_traversal(maze, current_x + 1, current_y)
        # Check up
        if self.finish_flag is not True and maze[current_x - 1][current_y] == ".":
            self._maze_traversal(maze, current_x - 1, current_y)
        # Check left
        if self.finish_flag is not True and maze[current_x][current_y - 1] == ".":
            self._maze_traversal(maze, current_x, current_y - 1)
        # Check right
        if self.finish_flag is not True and maze[current_x][current_y + 1] == ".":
            self._maze_traversal(maze, current_x, current_y + 1)
        # Changes to O if at dead end or wrong path
        if self.finish_flag is not True:
        ...
        for row in maze:
            ...
            for symbol in row:
                ...
                # Prints other symbols normally
                    ...
                    print(symbol, end=" ")
        # Prints final new line to separate the different mazes
        print()
